...
class MultiTaskModel(nn.Module):
    def __init__(self, base_model_name, num_sentiment_labels, num_task_labels, alpha=0.4, beta=0.6):
...
class TextRequest(BaseModel):
    texts: List[str]  # Danh sách các câu cần phân loại
...
def load_model_and_tokenizer(model_path="./models/phobert_multitask_best_model_V2.pt", tokenizer_path="./models/phobert_multitask_tokenizer"):
...
# Hàm dự đoán batch
def predict_batch(texts: List[str], model, tokenizer, device, max_len=128):
    ...
    for i, text in enumerate(texts):
        result = {
            "text": text,
            "sentiment": SENTIMENT_MAP.get(sentiment_preds[i], "Unknown"),
            "sentiment_confidence": float(sentiment_confidences[i]),
            "task": TASK_MAP.get(task_preds[i] + 1, "Unknown"),  # +1 vì task trong TASK_MAP bắt đầu từ 1
...
async def predict_texts(request: TextRequest):
    try:
        ...
        logger.info(f"Nhận yêu cầu phân loại cho {len(request.texts)} câu")
        predictions = predict_batch(request.texts, model, tokenizer, device)
